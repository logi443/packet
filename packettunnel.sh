...
function prompt_port_json() {
    ...
    if [[ -z "$input" ]]; then
        [[ "$allow_empty" == "true" ]] && echo "SKIP" || echo ""
        return
    fi
    ...
    if [[ "$input" == *","* ]]; then
        # Multiple ports - validate each and build JSON array
        local json_arr="["
        local first=true
...
function install_bitswap() {
    ...
    if [[ "$role" == "1" ]]; then
        ...
        port_listen_json="$(prompt_port_json "Enter listen port(s)")"
        [[ -z "$port_listen_json" ]] && { pause_return_menu; return; }
        ...
        echo
        echo "Final port options:"
        echo "  1) Single or multiple specific port(s)"
        echo "  2) Same as user's original port (src_context->port)"
        read -rp "Choose [1-2]: " final_mode

        local final_port_json
        if [[ "$final_mode" == "2" ]]; then
            final_port_json='"src_context->port"'
        else
            final_port_json="$(prompt_port_json "Enter final port(s)")"
            [[ -z "$final_port_json" ]] && { pause_return_menu; return; }
        fi

        generate_core_json "$mtu_val"
        generate_bitswap_kharej_config "$ip_iran" "$ip_kharej" "$port_listen" "$final_port_json"
...
function change_ports_bitswap() {
    ...
    # Find all nodes that have a port field in settings
    mapfile -t PORT_NODES < <(jq -r '
        .nodes[] | select(.settings.port != null)
        | "\(.name)|\(.type)|\(.settings.port)"
    ' "$CONFIG_FILE" 2>/dev/null)

    if [[ "${#PORT_NODES[@]}" -eq 0 ]]; then
        echo "No port fields found in config nodes."
        return
    fi

    for entry in "${PORT_NODES[@]}"; do
        local node_name="${entry%%|*}"
        local rest="${entry#*|}"
        local node_type="${rest%%|*}"
        local current_port="${rest#*|}"

        echo "Node: $node_name ($node_type)"
        echo "Current port: $current_port"

        local new_port_json
        new_port_json="$(prompt_port_json "Enter new port(s) (or press Enter to keep current)" "true")"

        if [[ "$new_port_json" == "SKIP" || -z "$new_port_json" ]]; then
            echo "Keeping current value."
        else
            tmp="$(mktemp)"
            jq --arg name "$node_name" --argjson port "$new_port_json" '
                (.nodes[] | select(.name == $name)).settings.port = $port
            ' "$CONFIG_FILE" > "$tmp"
            mv -f "$tmp" "$CONFIG_FILE"
            echo "Updated $node_name port to: $new_port_json"
        fi
        echo "----------------------------------------"
    done
}
